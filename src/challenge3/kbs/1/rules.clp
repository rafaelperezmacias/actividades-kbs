...
( defrule note-12-visa-rule 
    ...
    (test (> ?a 0))
    =>
    (assert (result (customer-id ?cid) (offer 13M) (description "13 meses sin intereses") (product-id 4) (price ?p) (order-id ?on) (amount ?a)))
)
...
( defrule macbook-iphone-rule
    ...
    (test (> ?ai 0))
    =>
    (assert (result (customer-id ?cid) (offer 110vx1200c) (description "110 pesos en vales x cada 1200 pesos de compra") (product-id 1) (price ?p2) (order-id ?on) (amount ?ai)))
    (assert (result (customer-id ?cid) (offer 110vx1200c) (description "110 pesos en vales x cada 1200 pesos de compra") (product-id 3) (price ?p1) (order-id ?on) (amount ?am)))
)
...
( defrule smartphone-f-m-rule
    ...
    (test (> ?a 0))
    => 
    (assert (result (customer-id ?cid) (offer FyM8%) (description "Funda y mica con 8% de descuento") (product-id ?pid) (price ?p) (order-id ?on) (amount 1)))
)